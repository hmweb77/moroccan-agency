...
const HeroContact = () => {
  return (
    <div className="relative min-h-[50vh] sm:min-h-[55vh] md:min-h-[60vh] lg:min-h-[65vh] xl:min-h-[70vh] 2xl:min-h-[75vh] overflow-hidden">
      ...
      <div className="relative mt-12 z-10 flex items-center justify-center min-h-[50vh] sm:min-h-[55vh] md:min-h-[60vh] lg:min-h-[65vh] xl:min-h-[70vh] 2xl:min-h-[75vh]">
        <div className="w-full max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 text-center">
          ...
          >
            ...
            >
              Prêt à transformer vos idées en réalité ? Notre équipe d'experts est là pour vous accompagner à chaque étape de votre projet.
            </motion.p>
            ...
            <motion.div
              ...
              className="flex flex-col xs:flex-col sm:flex-row gap-3 xs:gap-4 sm:gap-4 md:gap-5 lg:gap-6 justify-center items-center pt-4 xs:pt-5 sm:pt-6 md:pt-7 lg:pt-8 xl:pt-10 px-2 sm:px-0"
            >
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="w-full xs:w-full sm:w-auto bg-white text-blue-700 px-4 xs:px-5 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-2 xs:py-2.5 sm:py-3 md:py-4 lg:py-5 xl:py-6 rounded-full font-bold text-xs xs:text-sm sm:text-base md:text-lg lg:text-xl hover:bg-blue-50 transition-all duration-300 shadow-lg hover:shadow-xl max-w-xs xs:max-w-sm sm:max-w-none"
              >
                Démarrer un projet
              </motion.button>
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="w-full xs:w-full sm:w-auto border-2 border-white text-white px-4 xs:px-5 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-2 xs:py-2.5 sm:py-3 md:py-4 lg:py-5 xl:py-6 rounded-full font-bold text-xs xs:text-sm sm:text-base md:text-lg lg:text-xl hover:bg-white hover:text-blue-700 transition-all duration-300 max-w-xs xs:max-w-sm sm:max-w-none"
              >
                Planifier un appel
              </motion.button>
            </motion.div>
      ...
      {/* Extra decorative elements for xl+ screens */}
      <motion.div
        animate={{ 
          y: [0, -10, 0],
          rotate: [0, -2, 0]
        ...
        className="hidden xl:block absolute bottom-1/3 left-1/5 w-4 h-4 2xl:w-6 2xl:h-6 bg-white/6 rounded-lg opacity-15"
      />
    </div>
  );
};
